...
FLOOKUP = '../input/IdLookupTable.csv'
...
def load(test=False, cols=None):
    ...
    return X,y  # run the full dataset
...
def fit_specialists(fname_pretrain=None,column=None):
    ...
    if column:
        ...
        print("error: kfkd_v1 only trains one column at a time\n")
        print("usage:")
        print("python kfkd_v1.py fit_specialists net.pickle x \n")
        print("where x should be 0,1,2,3,4,5")
        assert(False)

    specialists = OrderedDict()
    
    # train one column at a time to relax memory pressure
    # load previous trained specialist
    if column>0:
        specialists = pickle.load(open('net-specialists_%d.pickle'%(column-1)))

    
    for setting in SPECIALIST_SETTINGS[column:column+1]: 
    ...
    # save the specialists for the columns so far
    with open('net-specialists_%d.pickle'%column, 'wb') as f:
